...
# 2. Precompute
if (!file.exists(paste0("/path/to/",tissue,"/sc-eQTL/results/functional_annotation/chromHMM/precomputed_matching_controls_",ancestry,"_5en8.rds"))){
  # Load gene annotations
  gene_annotations = fread(paste0("/path/to/resource/genes.bed"), select=c(1,3,4))
  ...
  background_path = paste0("/path/to/",tissue,"/sc-eQTL/genotype/",tissue,"_", ancestry, "_indivpruned.frq")
  ...
  # Load eSNPs
  index_snps = fread(paste0("/path/to/",tissue,"/sc-eQTL/results/functional_annotation/chromHMM/",tissue,"_", ancestry, "_all_esnp_5en8.bed"), select=c(1,3,4))
  ...
  saveRDS(precomputed, paste0("/path/to/",tissue,"/sc-eQTL/results/functional_annotation/chromHMM/precomputed_matching_controls_", ancestry, "_5en8.rds"))
  rm(background_snps)
}else{
  precomputed = readRDS(paste0("/path/to/",tissue,"/sc-eQTL/results/functional_annotation/chromHMM/precomputed_matching_controls_", ancestry, "_5en8.rds"))
}

# 3. Perform functional enrichment for each annotation
annotation_list = c("BivFlnk","Enh","EnhBiv","EnhG","Het","Quies","ReprPC","ReprPCWk","TssA","TssAFlnk","TssBiv","Tx","TxFlnk","TxWk","ZNF")
a = annotation_list[as.numeric(args[5])]
chromHMM_dir = args[6]
print(a)
chromatin_annotation=fread(paste0("/path/to/",tissue,"/sc-eQTL/results/functional_annotation/chromHMM/",tissue,"_",ancestry,"_null_",chromHMM_dir,"_",a,"_overlaps.bed"), select=4)
colnames(chromatin_annotation) = "variant_id"
chromatin_annotation[, Annotation := a ]
...
write.table(func_enrich, paste0("/path/to/crossTissue_analysis/functional_enrichment_5en8/functional_enrichment_chromHMM_",ancestry,"_5en8_",tissue,"_",chromHMM_dir,"_",a, ".txt"), row=F,quo=F,sep='\t',col=T)
